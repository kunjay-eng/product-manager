...
function _buildFastDetailData(ticker, market, cfg) {
  ...
  const pivot = _getPivotSafe(ticker);
  const pivotTakeaways = buildPivotKeyTakeaways(pivot, sig.price);
  ...
  return {
    ...
    checklist,
    checklistCaption,
      support, // { available, currentPrice, ... }
    resistance, // { available, currentPrice, ... }
    pivot, // ← เพิ่ม
    riskManagement: {
    ...
    recommendation: {
      ...
      reasons: sig.reasonsFor.concat(pivotTakeaways.reasons),      // ← แก้
      warnings: sig.reasonsAgainst.concat(pivotTakeaways.warnings), // ← แก้
...
function _buildPortfolioDetailData(ticker, market, cfg) {
  ...
  return {
    ...
    rebalance: rebalanceAction,
       support: _getSupportSafe(ticker),
    resistance: _getResistanceSafe(ticker),
    pivot: _getPivotSafe(ticker), // ← เพิ่ม
    recommendation: _buildPortfolioRecommendation(decision, weightInfo, holding, _getPivotSafe(ticker), holding.priceNow), // ← เพิ่ม 2 arg ท้าย
...
// ── เรียก getPivotLevels() แบบกันพัง เช่นเดียวกับ _getSupportSafe/_getResistanceSafe ──
function _getPivotSafe(ticker) {
  try {
    return getPivotLevels(ticker);
  } catch (e) {
    return { available: false, reason: 'ไม่มีข้อมูล (เช็คว่ามีไฟล์ webapp_09_external_history.gs และตั้งค่า EXTERNAL_LOG_SHEET_ID แล้วหรือยัง)' };
  }
}
...
function _buildPortfolioRecommendation(decision, weightInfo, holding, pivot, currentPrice) {
  const reasons = [];
  ...
  const pivotTakeaways = buildPivotKeyTakeaways(pivot, currentPrice);
  return {
    verdict: decision,
    reasons: reasons.concat(pivotTakeaways.reasons),
    warnings: pivotTakeaways.warnings,
    actions
  };
}
